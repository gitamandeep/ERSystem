...
const Employeedatapage = () => {
    ...
    const handleDeleteExpense = async () => {
        if (itemData.status === 'PENDING') {
            try {
                // Uncomment and use this code to delete the item from the server
                // await axios.delete(`/api/expenses/${itemData.id}`);
                // For demo purposes, simulate removing the item from the local data
                data.splice(data.findIndex(item => item.id === id), 1);
                setItemData(null);  // Clear the state to reflect that the item has been deleted
                navigate('/employeetable');  // Navigate back to the main page or wherever you want after deletion
            } catch (error) {
                console.error('Error deleting expense:', error);  // Handle errors
            }
        } else {
            alert('You can only delete expenses with a "PENDING" status.');
        }
    };
    ...
    return (
        <div className="flex flex-col min-h-screen bg-gray-100">
            <EmployeeNavbar />  {/* Render the Navbar component */}
                ...
                <div className="max-w-6xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden">
                    <div className="p-8">
                        ...
                        <div className="mt-6 flex gap-4">
                            {/* Delete button */}
                            <button
                                onClick={handleDeleteExpense}  // Handle deletion on click
                                className={`bg-red-500 text-white rounded-lg py-2 px-4 text-center font-medium hover:bg-red-600 transition duration-300 ease-in-out ${itemData.status !== 'PENDING' ? 'opacity-50 cursor-not-allowed' : ''}`}
                                disabled={itemData.status !== 'PENDING'}  // Disable button if status is not 'PENDING'
                            >
                                Delete Expense
                            </button>
                        </div>
...
export default Employeedatapage;  // Export the Datapage component for use in other parts of the application
